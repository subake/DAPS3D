...
from tasks.semantic.modules.module_helper import ModuleHelper
...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=True, corners=True):
        ...
        self.oc_module_pre = nn.Sequential(
            nn.Conv2d(oc_size, oc_size, kernel_size=3, stride=1, padding=1),
            ModuleHelper.BNReLU(oc_size, bn_type='inplace_abn'),
        )
        
        from tasks.semantic.modules.oc_module import BaseOC_Module
        self.oc_module = BaseOC_Module(in_channels=oc_size, 
                                       out_channels=oc_size,
                                       key_channels=oc_size // 2, 
                                       value_channels=oc_size // 2,
                                       dropout=0.05, 
                                       sizes=([1]),
                                       bn_type='inplace_abn')
