...
from tasks.semantic.modules.segformer_modules.layers import DropPath
...
class PatchEmbed(nn.Module):
    def __init__(self, c1=3, c2=32, patch_size=7, stride=4):
        ...
        if type(patch_size) is tuple:
            pad = (patch_size[0]//2, patch_size[1]//2)
        else:
            pad = patch_size//2

        self.proj = nn.Conv2d(c1, c2, patch_size, stride, pad)    # padding=(ps[0]//2, ps[1]//2)
...
class MiT(nn.Module):
    def __init__(self, model_name: str = 'B0'):
        ...
        # patch_embed
        self.patch_embed1 = PatchEmbed(4, embed_dims[0], 3, 2)  # (3, 7), (2, 4)
