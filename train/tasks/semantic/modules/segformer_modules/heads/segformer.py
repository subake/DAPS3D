...
class SegFormerHead(nn.Module):
    ...
    def forward(self, features: Tuple[Tensor, Tensor, Tensor, Tensor]) -> Tensor:
        ...
        outs[0] = F.interpolate(outs[0], size=(H_new, W_new), mode='bilinear', align_corners=False)

        for i, feature in enumerate(features[1:]):
            cf = eval(f"self.linear_c{i+2}")(feature).permute(0, 2, 1).reshape(B, -1, *feature.shape[-2:])
            # outs.append(F.interpolate(cf, size=(H, W), mode='bilinear', align_corners=False))
            outs.append(F.interpolate(cf, size=(H_new, W_new), mode='bilinear', align_corners=False))
