...
class KNN(nn.Module):
    ...
    def forward(self, proj_range, unproj_range, proj_argmax, px, py):
        ...
        # NOTE: patch for ONNX conversion
        # ones = torch.ones_like(knn_argmax_onehot).type(proj_range.type())
        # workaround for w += input_x
        # knn_argmax_onehot = torch.scatter(knn_argmax_onehot, 1,
        #                                   knn_argmax, knn_argmax_onehot + ones)
